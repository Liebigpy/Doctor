...
from research.models.trajectory_gpt2 import GPT2Model
import math
import numpy as np
...
class DecisionTransformer(TrajectoryModel):
    ...
    def forward(
        ...
        #将嵌入的状态、动作和回报按照序列顺序叠加起来，
        # 并通过一个 Layer Normalization（embed_ln）进行处理。
        # 这样的叠加使得模型在预测时能够利用上一个状态和动作来预测下一个回报。
        #输出应该是 (batch_size, 3 * seq_length, hidden_size)
        ...
        # 使用注意力机制（transformer）处理嵌入的序列，
        # 模型可以关注输入序列中不同位置的信息，提高模型的表示能力。
